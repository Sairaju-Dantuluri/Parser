...
class Token:
    # Contains the TK_identifier
    token = ""
    # Contains the actual lexeme
    lexeme = ""
    # Contains the start index of lexeme
    begin = 0
    # Contains end index of lexeme
    end = 0
    # Contains line number of lexeme
    line = 0
...
class Tokenizer:
    # Bool that keeps track whether currently pointer is in single line comment
    in_singleline_comment = False
    # Keeps track whether pointer in multiline comment
    in_multiline_comment = False
    state = "start"                                        # Keeps track of DFA state
    # Keeps track of previously found valid Token
    ...
    def get_token_list(self, filename):
        ...
        for line in file:
            ...
            # print(line)
            # print()
            # Reset start state to "start" at the beginning of each line buffer
            self.state = "start"
            self.get_tokens(ascii_list, line_number)
            line_number += 1
            if self.in_singleline_comment:
                # Moving to next line if current is singleline comment
    ...
    def get_tokens(self, chars, line_number):
        ...
        while self.fp < len(chars):
            ...
            if self.in_multiline_comment:
                q = self.is_completely_multiline_comment(
                    chars)        # Handling Multiline comment
                # print(q)
                if q:
                    return
                else:
                    continue
            # Skip current line iteration if in single-line comment
            if self.in_singleline_comment:
                return
            if self.state == "Error":                                  # Store Error Token
                lexeme_ = str(''.join(chr(chars[i])
        ...
        # If line ends in incomplete lexeme, throw "Invalid_Error" Error and store
        if not self.in_multiline_comment and self.lb != self.fp:
            # print("Error")
            lexeme_ = str(''.join(chr(chars[i])
                          for i in range(self.lb, self.fp)))
            self.token_list.append(
                Token("Invalid_Token", lexeme_, self.lb, self.fp, line_number))

    # Iterate fp until end of multiline comment
    ...
    # Method that changes the current state in DFA
    def change_state(self, char, chars):
        ...
        if self.state == "start":                                      # If current state of DFA is "start"
            ...
            if chr(char).isalnum() or char == ord('_'):
                if self.fp+1 < len(chars) and (chr(chars[self.fp+1]).isalnum() or chars[self.fp+1] == ord('_')):
                    ...
                    is_final_state = True
                    incomplete_lexeme = str(
                        ''.join(chr(chars[i]) for i in range(self.lb, self.fp+1)))
        ...
        # If current state of DFA is "float_literal"
        ...
        # If none DFA in none of the above state,..
        self.state = "Error"
        # ...move to sink state which is an "Error"
        return self.state, is_final_state

    # Retract method that decrements (retracts)...
    def retract(self):
        # ...fp by one.
        self.fp -= 1
